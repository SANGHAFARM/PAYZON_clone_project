...
<title>退職処理</title>
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/css/retirement/retirement-process.css?v=20260821-1">
<link rel="stylesheet"
...
<body>
	...
	<main class="retirement-page page-content">
		...
		<section class="retirement-card">
			<div class="retirement-toolbar">
				<form class="retirement-keyword-search" method="get"
					action="${pageContext.request.contextPath}/retirement/process.do">
					<select name="searchTarget" aria-label="検索項目">
						<option value="name"
							${param.searchTarget eq 'name' ? 'selected' : ''}>氏名</option>
						<option value="employeeNo"
							${param.searchTarget eq 'employeeNo' ? 'selected' : ''}>社員番号</option>
						<option value="department"
							${param.searchTarget eq 'department' ? 'selected' : ''}>部署</option>
						<option value="all"
							${param.searchTarget eq 'all' ? 'selected' : ''}>全体</option>
					</select> <input type="search" name="keyword" value="${param.keyword}"
						placeholder="検索キーワードを入力" aria-label="検索語">
					<button type="submit" class="search-button">検索</button>
					<a class="all-view"
						href="${pageContext.request.contextPath}/retirement/process.do">全体を見る</a>
					<select class="status-filter" name="status" aria-label="在職状況">
						<option value="">ステータス別</option>
						<option value="ACTIVE"
							${param.status eq 'ACTIVE' ? 'selected' : ''}>在職</option>
						<option value="RETIRED"
							${param.status eq 'RETIRED' ? 'selected' : ''}>退職</option>
					</select>
				...
				<table class="retirement-table data-table">
					...
					<tbody>
						<c:forEach var="employee" items="${employees}" varStatus="status">
							<tr class="employee-row">
								...
								<td><a href="#retirement-${employee.employeeId}"
									class="employee-status ${employee.status eq 'RETIRED' ? 'is-retired' : 'is-active'}"><ui:code-label
											value="${employee.statusName}" /></a></td>
						...
						<c:if test="${empty employees}">
							<tr>
								<td colspan="11" class="empty-row">検索された社員はありません。</td>
							</tr>
						</c:if>
					</tbody>
				</table>
			...
			<div class="pagination">
				<c:if test="${pageInfo.hasPrevious}">
					<c:url var="previousUrl" value="/retirement/process.do">
						...
						<c:param name="status"
							value="${condition.status eq 'WORK' ? 'ACTIVE' : condition.status eq 'RETIRED' ? 'RETIRED' : ''}" />
					</c:url>
					<a href="${previousUrl}">以前</a>
				</c:if>
				<c:forEach var="pageNo" begin="${pageInfo.startPage}"
					end="${pageInfo.endPage}">
					<c:url var="pageUrl" value="/retirement/process.do">
					...
					<a class="${pageNo eq pageInfo.currentPage ? 'is-current' : ''}"
						href="${pageUrl}">${pageNo}</a>
				</c:forEach>
				<c:if test="${pageInfo.hasNext}">
					<c:url var="nextUrl" value="/retirement/process.do">
		...
		<c:forEach var="employee" items="${employees}">
			<section id="retirement-${employee.employeeId}"
				class="retirement-modal-overlay">
				<div class="retirement-modal" role="dialog" aria-modal="true"
					aria-labelledby="title-${employee.employeeId}">
					<header>
					...
					<form method="post"
						...
						<input type="hidden" name="employeeId"
							...
							name="processType"
							value="${employee.status eq 'RETIRED' ? 'CANCEL' : 'RETIRE'}">
						<div class="retirement-modal__body">
							<c:choose>
								<c:when test="${employee.status eq 'RETIRED'}">
									<label><span>退職区分</span><select disabled
										aria-disabled="true"><option>${employee.retirementTypeName}</option></select></label>
									<input type="hidden" name="retirementType"
										value="${employee.retirementType}">
									<label><span>退職日</span><input type="date" lang="ja-JP"
										name="retirementDate" value="${employee.retirementDate}"
										readonly></label>
								</c:when>
								<c:otherwise>
									<label><span>退職区分</span><select name="retirementType"><option
												value="">選択</option>
											<c:forEach var="type" items="${retirementTypes}">
												<option value="${type.code}">${type.name}</option>
											</c:forEach></select></label>
									<label><span>退職日</span><input type="date" lang="ja-JP"
										name="retirementDate"
										value="${empty employee.retirementDate ? currentDate : employee.retirementDate}"></label>
								</c:otherwise>
							</c:choose>
							<label><span>退職理由</span><input type="text"
								name="retirementReason" value="${employee.retirementReason}"
								${employee.status eq 'RETIRED' ? 'readonly' : ''}></label> <label><span>退職後の連絡先</span><input
								type="text" name="afterContact" value="${employee.afterContact}"
								${employee.status eq 'RETIRED' ? 'readonly' : ''}></label>
						</div>
						<div class="retirement-modal__actions">
							<button type="submit" class="button button-primary">
								${employee.status eq 'RETIRED' ? '退職処理取消' : '退職処理'}</button>
						</div>
					</form>
				</div>
			</section>
	...
	<c:if test="${not empty retirementPopupMessage or not empty message}">
		<div class="retirement-alert" role="alertdialog" aria-modal="true"
			aria-labelledby="retirement-alert-message">
			<a class="retirement-alert__backdrop"
				href="${pageContext.request.contextPath}/retirement/process.do"
				aria-label="案内を閉じる"></a>
			<div class="retirement-alert__panel">
				<p id="retirement-alert-message">
					<ui:message-label
						value="${not empty retirementPopupMessage ? retirementPopupMessage : message}" />
				</p>
