			...
			<section class="employee-card">
				...
				<form action="${pageContext.request.contextPath}/personnel/employee-action.do" method="post">
					...
					<div class="pagination">
						<c:if test="${pageInfo.hasPrevious}"><c:url var="previousUrl" value="/personnel/employees.do"><c:param name="page" value="${pageInfo.previousPage}"/><c:param name="pageSize" value="${condition.pageSize}"/><c:param name="searchTarget" value="${condition.searchTarget}"/><c:param name="keyword" value="${condition.keyword}"/><c:param name="employmentType" value="${condition.employmentType}"/><c:param name="status" value="${condition.status}"/></c:url><a href="${previousUrl}">이전</a></c:if>
						<c:forEach var="pageNo" begin="${pageInfo.startPage}" end="${pageInfo.endPage}"><c:url var="pageUrl" value="/personnel/employees.do"><c:param name="page" value="${pageNo}"/><c:param name="pageSize" value="${condition.pageSize}"/><c:param name="searchTarget" value="${condition.searchTarget}"/><c:param name="keyword" value="${condition.keyword}"/><c:param name="employmentType" value="${condition.employmentType}"/><c:param name="status" value="${condition.status}"/></c:url><a class="${pageNo eq pageInfo.currentPage ? 'is-current' : ''}" href="${pageUrl}">${pageNo}</a></c:forEach>
						<c:if test="${pageInfo.hasNext}"><c:url var="nextUrl" value="/personnel/employees.do"><c:param name="page" value="${pageInfo.nextPage}"/><c:param name="pageSize" value="${condition.pageSize}"/><c:param name="searchTarget" value="${condition.searchTarget}"/><c:param name="keyword" value="${condition.keyword}"/><c:param name="employmentType" value="${condition.employmentType}"/><c:param name="status" value="${condition.status}"/></c:url><a href="${nextUrl}">다음</a></c:if>
					</div>
