            ...
            <div class="certificate-layout">
                ...
                <section class="certificate-workspace">
                    ...
                    <form class="certificate-form" id="certificate-issue-form" action="${pageContext.request.contextPath}/employees/certificate-issue.do" method="post">
                        ...
                        <textarea class="certificate-message" name="certificateMemo"><c:choose><c:when test="${selectedCertificateType eq 'WORKING'}">上記の者は現在、上記のように当社に在職していることを証明します。</c:when><c:when test="${selectedCertificateType eq 'CAREER'}">上記の者に対しての当社の経歴事項を上記のように証明します。</c:when><c:when test="${selectedCertificateType eq 'RETIREMENT'}">上記の者は、当社に上記のように在職したことを証明します。</c:when><c:otherwise>上記の者は現在、上記のように当社に在職していることを証明します。</c:otherwise></c:choose></textarea>
