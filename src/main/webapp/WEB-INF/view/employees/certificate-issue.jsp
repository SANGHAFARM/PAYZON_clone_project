				...
				<section class="certificate-workspace">
					...
					<form class="certificate-form" id="certificate-issue-form" action="${pageContext.request.contextPath}/personnel/certificate-issue.do" method="post">
						...
						<div class="certificate-document certificate-document--working">
							...
							<header class="document-title"><div class="company-logo"><c:choose><c:when test="${not empty company.logoImgPath}"><img src="<c:out value='${company.logoImgPath}' />" alt="회사 로고"></c:when><c:otherwise><span>회사 로고</span></c:otherwise></c:choose></div><h2>재 직 증 명 서</h2><label><input type="checkbox" name="hideResidentNoWorking" value="Y"> 주민등록번호 숨김</label></header>
						...
						<div class="certificate-document certificate-document--career">
							...
							<header class="document-title"><div class="company-logo"><c:choose><c:when test="${not empty company.logoImgPath}"><img src="<c:out value='${company.logoImgPath}' />" alt="회사 로고"></c:when><c:otherwise><span>회사 로고</span></c:otherwise></c:choose></div><h2>경 력 증 명 서</h2><label><input type="checkbox" name="hideResidentNoCareer" value="Y"> 주민등록번호 숨김</label></header>
						...
						<div class="certificate-document certificate-document--retirement">
							...
							<header class="document-title"><div class="company-logo"><c:choose><c:when test="${not empty company.logoImgPath}"><img src="<c:out value='${company.logoImgPath}' />" alt="회사 로고"></c:when><c:otherwise><span>회사 로고</span></c:otherwise></c:choose></div><h2>퇴 직 증 명 서</h2><label><input type="checkbox" name="hideResidentNoRetirement" value="Y"> 주민등록번호 숨김</label></header>
						...
						<footer class="certificate-footer"><label><input type="checkbox" name="showCeo" value="Y" checked> 대표자 표기</label><div><strong><c:out value="${company.cmpnName}" /></strong><span><c:out value="${company.ceoTitle}" /> <c:out value="${company.ceoName}" /></span></div><div class="stamp-box"><c:choose><c:when test="${not empty company.stampImgPath}"><img src="<c:out value='${company.stampImgPath}' />" alt="회사 도장"></c:when><c:otherwise><span>회사 도장을<br>넣어주세요</span></c:otherwise></c:choose></div></footer>
