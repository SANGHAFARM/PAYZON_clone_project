...
<body>
	...
	<main class="page-content holiday-page">
		...
		<section class="holiday-card">
			<form class="holiday-search"
				action="${pageContext.request.requestURI}" method="get">
				<select name="holidayItemId" aria-label="휴가항목"><option
						value="">휴가항목 선택</option>
					<c:forEach var="item" items="${leaveItems}">
						<option value="${item.leaveItemId}"
							${item.leaveItemId eq leaveitemId ? 'selected' : ''}><c:out
								value="${item.itemName}" /></option>
					</c:forEach></select> <input type="search" name="keyword" value="${keyword}"
					placeholder="검색어 입력">
				<button type="submit">검색</button>
				<a href="${pageContext.request.contextPath}/attendance/leave-inquiry.do">전체보기</a>
				<div class="filter-group">
					<select name="status" aria-label="상태별"><option value="">상태별</option>
						<option value="재직">재직</option>
						<option value="퇴직">퇴직</option></select>
						 <select name="empType"
						aria-label="구분별"><option value="">구분별</option>
						...
						</c:forEach></select> <select name="positionId" aria-label="직위별"><option
							value="">직위별</option>
						<c:forEach var="jobPosition" items="${jobPositions}">
							<option value="${jobPosition.jobPositionId}"><c:out
									value="${jobPosition.jobPositionName}" /></option>
						</c:forEach></select> <select name="pageSize" aria-label="목록 수"><option
							value="10">10개씩 보기</option>
				...
				<table class="holiday-table">
					...
					<tbody>
						<c:forEach var="employee" items="${holidayEmployees}">
							...
							<tr>
								<td><a href="#${modalId}"><c:out
											value="${employee.employmentType}" /></a></td>
								<td><a href="#${modalId}"><c:out
											value="${employee.employeeNo}" /></a></td>
								<td><a href="#${modalId}"><c:out
											value="${employee.name}" /></a></td>
								<td><a href="#${modalId}"><c:out
											value="${employee.departmentName}" /></a></td>
								<td><a href="#${modalId}"><c:out
											value="${employee.positionName}" /></a></td>
								<td><a href="#${modalId}"><c:out
											value="${employee.holidayName}" /></a></td>
								<td class="total-days"><a href="#${modalId}"><c:out
											value="${employee.totalDays}" /></a></td>
								<td class="used-days"><a href="#${modalId}"><c:out
											value="${employee.usedDays}" /></a></td>
								<td class="remaining-days"><a href="#${modalId}"><c:out
	...
	<c:forEach var="employee" items="${holidayEmployees}">
		<div class="holiday-modal-overlay"
			id="holiday-modal-${employee.employeeId}">
			<section class="holiday-modal" role="dialog" aria-modal="true"
				aria-labelledby="holiday-title-${employee.employeeId}">
				<header>
					<h2>사원별 휴가현황</h2>
					<a href="#" aria-label="닫기">&times;</a>
				</header>
				<div class="holiday-modal-body">
					<h3 id="holiday-title-${employee.employeeId}">
						[
					...
					<table>
						...
						<tbody>
							<c:forEach var="record" items="${employee.holidayRecords}">
								<tr>
									<td><c:out value="${record.rowNumber}" /></td>
									<td><c:out value="${record.inputDate}" /></td>
									<td><c:out value="${record.holidayName}" /></td>
									<td><c:out value="${record.attendanceItemName}" /></td>
									<td><c:out value="${record.period}" /></td>
									<td class="record-days"><c:out value="${record.days}" /></td>
									<td><c:out value="${record.note}" /></td>
								</tr>
							</c:forEach>
							<c:if test="${empty employee.holidayRecords}">
								<tr>
						...
						<tfoot>
							<tr>
								...
								<td colspan="5"><span>총 휴가일수 : <strong><c:out
												value="${employee.totalDays}" /></strong></span><span>사용일수 : <strong
										class="used-days"><c:out value="${employee.usedDays}" /></strong></span><span>잔여일수
										: <strong class="remaining-days"><c:out
												value="${employee.remainingDays}" /></strong>
								</span></td>
							</tr>
						</tfoot>
					</table>
