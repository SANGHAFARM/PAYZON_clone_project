		...
		<section class="holiday-card">
			<form class="holiday-search"
				...
				<select name="leaveItemId" aria-label="休暇項目"
					onchange="if(this.value === '') { location.href='${pageContext.request.contextPath}/settings/attendance.do'; }">
					<!-- 휴가항목을 선택 클릭 시 기본 설정의 휴가/근태 설정으로 리다이렉트 -->
					<!-- 休暇項目を選択をクリックすると、基本設定の休暇・勤怠設定にリダイレクト -->
			...
			<div class="holiday-table-wrap">
				<table class="holiday-table">
					...
					<tbody>
						<c:forEach var="balance" items="${leaveInquiryPage.content}">
							<c:set var="detailUrl"
								value="${pageContext.request.contextPath}/attendance/leave-inquiry.do?leaveItemId=${leaveItemId}&keyword=${keyword}&status=${status}&empType=${empType}&departmentId=${departmentId}&jobPositionId=${jobPositionId}&pageSize=${pageSize}&page=${leaveInquiryPage.currentPage}&employeeId=${balance.employeeId}#holiday-modal-${balance.employeeId}" />
							<tr>
								<td><a href="${detailUrl}"><ui:code-label
											value="${balance.empType}" /></a></td>
								<td><a href="${detailUrl}"><c:out
											value="${balance.empNo}" /></a></td>
								<td><a href="${detailUrl}"><c:out
											value="${balance.empNameKr}" /></a></td>
								<td><a href="${detailUrl}"><c:out
											value="${balance.departmentName}" /></a></td>
								<td><a href="${detailUrl}"><c:out
											value="${balance.jobPositionName}" /></a></td>
								<td><a href="${detailUrl}"><c:out
											value="${balance.itemName}" /></a></td>
								<td class="total-days"><a href="${detailUrl}"><c:out
											value="${balance.totalDays}" /></a></td>
								<td class="used-days"><a href="${detailUrl}"><c:out
											value="${balance.usedDays}" /></a></td>
								<td class="remaining-days"><a href="${detailUrl}"><c:out
											value="${balance.remainingDays}" /></a></td>
							</tr>
						</c:forEach>
						<c:if test="${empty leaveInquiryPage.content}">
							<tr>
			...
			<c:if
				test="${not empty leaveInquiryPage and leaveInquiryPage.total > 0}">
				<nav class="pagination" aria-label="ページ移動">
					<c:if test="${leaveInquiryPage.startPage > 10}">
						<a
							href="?leaveItemId=${leaveItemId}&keyword=${keyword}&status=${status}&empType=${empType}&departmentId=${departmentId}&jobPositionId=${jobPositionId}&pageSize=${pageSize}&page=${leaveInquiryPage.startPage - 10}">[이전]</a>
					</c:if>
					<c:forEach var="pNo" begin="${leaveInquiryPage.startPage}"
						end="${leaveInquiryPage.endPage}">
						<c:choose>
							<c:when test="${pNo eq leaveInquiryPage.currentPage}">
								<strong
									style="margin: 0 4px; color: #007bff; font-weight: bold;">${pNo}</strong>
							</c:when>
							<c:otherwise>
								<a
									href="?leaveItemId=${leaveItemId}&keyword=${keyword}&status=${status}&empType=${empType}&departmentId=${departmentId}&jobPositionId=${jobPositionId}&pageSize=${pageSize}&page=${pNo}"
									style="margin: 0 4px;">${pNo}</a>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<c:if
						test="${leaveInquiryPage.endPage < leaveInquiryPage.totalPages}">
						<a
							href="?leaveItemId=${leaveItemId}&keyword=${keyword}&status=${status}&empType=${empType}&departmentId=${departmentId}&jobPositionId=${jobPositionId}&pageSize=${pageSize}&page=${leaveInquiryPage.startPage + 10}">[다음]</a>
					</c:if>
				</nav>
			</c:if>
		</section>
		...
		<div class="holiday-modal-overlay" id="holiday-modal-${employeeId}">
			<section class="holiday-modal" role="dialog" aria-modal="true"
				...
				<header>
					<h2>社員別の休暇状況</h2>
					<a
						href="${pageContext.request.contextPath}/attendance/leave-inquiry.do?leaveItemId=${leaveItemId}&keyword=${keyword}&status=${status}&empType=${empType}&departmentId=${departmentId}&jobPositionId=${jobPositionId}&pageSize=${pageSize}&page=${leaveInquiryPage.currentPage}"
