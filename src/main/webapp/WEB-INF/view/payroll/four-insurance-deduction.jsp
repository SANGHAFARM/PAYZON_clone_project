    ...
    <main class="page-content four-insurance-page">
        ...
        <section class="insurance-card">
            <form class="insurance-search" method="get" action="${pageContext.request.contextPath}/payroll/four-insurance.do">
                ...
                <div class="search-field round-field"><label for="paymentRound">給与回次</label><select id="paymentRound" name="round"><option value="">回次を選択</option><c:forEach var="round" begin="1" end="10"><option value="${round}" <c:if test="${round eq selectedRound}">selected</c:if>>給与 ${round}回</option></c:forEach></select></div>
