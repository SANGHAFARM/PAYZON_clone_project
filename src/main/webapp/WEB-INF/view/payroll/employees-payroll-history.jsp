...
<head>
	...
	<link rel="stylesheet" href="${pageContext.request.contextPath}/css/payroll/employees-payroll-history.css">
</head>
<body>
	...
	<main class="page-content employee-payment-history-page">
		...
		<section class="history-card">
			<form class="history-search" method="get" action="${pageContext.request.contextPath}/payroll/employee-history.do">
				...
				<div class="period-search-group"><div class="search-field period-field"><label for="startMonth">기간선택</label><div class="month-range"><input id="startMonth" name="startMonth" type="month" value="${startMonth}"><span>~</span><input id="endMonth" name="endMonth" type="month" value="${endMonth}"></div></div><button type="submit" class="button button-primary period-submit">조회</button></div>
			</form>
			...
			<nav class="pagination" aria-label="급여내역 페이지">
				...
				<c:if test="${pageInfo.hasPrevious}"><a href="?page=${pageInfo.previousPage}&amp;startMonth=${startMonth}&amp;endMonth=${endMonth}&amp;employeeId=${selectedEmployee.employeeId}">이전</a></c:if>
				<c:forEach var="pageNumber" begin="${pageInfo.startPage}" end="${pageInfo.endPage}"><a class="${pageNumber eq pageInfo.currentPage ? 'active' : ''}" href="?page=${pageNumber}&amp;startMonth=${startMonth}&amp;endMonth=${endMonth}&amp;employeeId=${selectedEmployee.employeeId}">${pageNumber}</a></c:forEach>
				<c:if test="${pageInfo.hasNext}"><a href="?page=${pageInfo.nextPage}&amp;startMonth=${startMonth}&amp;endMonth=${endMonth}&amp;employeeId=${selectedEmployee.employeeId}">다음</a></c:if>
				</c:if>
			</nav>
