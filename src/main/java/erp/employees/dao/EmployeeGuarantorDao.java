...
public class EmployeeGuarantorDao {
    ...
    public void insert(Connection conn, EmployeeGuarantor guarantor) throws SQLException {
        ...
        try {
            ...
            pstmt.setLong(1, guarantor.getEmployeeId());
    ...
    // 보증인 내역 단건 조회
    // 기본키(EMPLOYEE_GUARANTOR_ID)를 기준으로 1건의 데이터 반환
    ...
    // 특정 사원의 보증인 내역 목록 조회
    // 사원번호(EMPLOYEE_ID)를 기준으로 연관된 보증인 내역 전체 반환 (등록순 정렬)
    public List<EmployeeGuarantor> selectByEmpId(Connection conn, int empId) throws SQLException {
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
            String sql = "SELECT EMPLOYEE_GUARANTOR_ID, EMPLOYEE_ID, GUARANTOR_NAME, RELATION, JUMIN_NO, GUARANTEE_AMT, GUARANTEE_DATE, EXPIRE_DATE, TEL_NO "
                       + "FROM EMPLOYEE_GUARANTOR WHERE EMPLOYEE_ID = ? ORDER BY EMPLOYEE_GUARANTOR_ID ASC";
    ...
    public int update(Connection conn, EmployeeGuarantor guarantor) throws SQLException {
        ...
        try {
            ...
            pstmt.setString(8, guarantor.getTelNo());
            pstmt.setLong(9, guarantor.getEmployeeGuarantorId());
    ...
    private EmployeeGuarantor makeEmpGuarantorFromResultSet(ResultSet rs) throws SQLException {
        EmployeeGuarantor guarantor = new EmployeeGuarantor();
        guarantor.setEmployeeGuarantorId(rs.getLong("EMPLOYEE_GUARANTOR_ID"));
        guarantor.setEmployeeId(rs.getLong("EMPLOYEE_ID"));
