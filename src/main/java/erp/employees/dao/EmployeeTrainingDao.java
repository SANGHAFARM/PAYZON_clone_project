...
public class EmployeeTrainingDao {
    ...
    public void insert(Connection conn, EmployeeTraining training) throws SQLException {
        ...
        try {
            ...
            pstmt.setLong(1, training.getEmployeeId());
    ...
    // 기본키(EMPLOYEE_TRAINING_ID)를 기준으로 1건의 데이터 조회
    public EmployeeTraining selectById(Connection conn, int trainId) throws SQLException {
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
    ...
    // 사원번호(EMPLOYEE_ID)를 기준으로 연관된 교육/훈련 내역 전체 반환 (최신 시작일 기준 내림차순 정렬)
    public List<EmployeeTraining> selectByEmpId(Connection conn, int empId) throws SQLException {
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
            String sql = "SELECT EMPLOYEE_TRAINING_ID, EMPLOYEE_ID, TRAIN_TYPE, TRAIN_NAME, START_DATE, END_DATE, TRAIN_INSTITUTE, TRAIN_COST, REFUND_COST "
                       + "FROM EMPLOYEE_TRAINING WHERE EMPLOYEE_ID = ? ORDER BY START_DATE DESC, EMPLOYEE_TRAINING_ID DESC";
    ...
    public int update(Connection conn, EmployeeTraining training) throws SQLException {
        ...
        try {
            ...
            pstmt.setLong(7, training.getTrainCost());
            pstmt.setLong(8, training.getRefundCost());
            pstmt.setLong(9, training.getEmployeeTrainingId());
    ...
    private EmployeeTraining makeEmpTrainingFromResultSet(ResultSet rs) throws SQLException {
        EmployeeTraining training = new EmployeeTraining();
        training.setEmployeeTrainingId(rs.getLong("EMPLOYEE_TRAINING_ID"));
        training.setEmployeeId(rs.getLong("EMPLOYEE_ID"));
