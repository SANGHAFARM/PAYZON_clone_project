...
public class EmployeeLicenseDao {
    ...
    public void insert(Connection conn, EmployeeLicense license) throws SQLException {
        ...
        try {
            ...
            pstmt.setLong(1, license.getEmployeeId());
    ...
    // 자격/면허 내역 단건 조회
    // 기본키(EMPLOYEE_LICENSE_ID)를 기준으로 1건의 데이터 반환
    ...
    // 사원번호(EMPLOYEE_ID)를 기준으로 연관된 자격증 내역 전체 반환 (최근 취득일 기준 내림차순 정렬)
    ...
    public int update(Connection conn, EmployeeLicense license) throws SQLException {
        ...
        try {
            ...
            pstmt.setString(5, license.getLicenseNo());
            pstmt.setString(6, license.getNote());
            pstmt.setLong(7, license.getEmployeeLicenseId());
    ...
    private EmployeeLicense makeEmpLicenseFromResultSet(ResultSet rs) throws SQLException {
        ...
        license.setEmployeeLicenseId(rs.getLong("EMPLOYEE_LICENSE_ID"));
        license.setEmployeeId(rs.getLong("EMPLOYEE_ID"));
