...
// 현장·프로젝트Delete 업무 규칙과 데이터 변경 트랜잭션을 처리한다.
// 現場・プロジェクトDeleteの業務ルールとデータ変更トランザクションを処理する。
public class ProjectDeleteService {
	private ProjectDao projectDao = ProjectDao.getInstance();
	
	// 선택되거나 식별된 현장·프로젝트Delete 데이터를 삭제하고 관련 상태를 정리한다.
	// 하나의 Connection에서 자동 커밋을 해제하고 관련 DAO 작업을 묶어 성공 시 commit, 실패 시 rollback한다.
	// 選択または識別された現場・プロジェクトDeleteデータを削除し、関連状態を整理する。
