...
public class LeaveBalanceListService {
	...
	public List<LeaveInquiryDto> getLeaveEmployees(List<Integer> employeeIds, int leaveItemId) {
		List<LeaveInquiryDto> result = new ArrayList<>();
		try (Connection conn = ConnectionProvider.getConnection()) {
			for (Integer employeeId : employeeIds) {
				result.addAll(employeeLeaveBalanceDao.selectLeaveBalance(conn, employeeId, leaveItemId));
			}
			return result;
