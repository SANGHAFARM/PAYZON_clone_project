...
public class LeaveItemDao {
	 ...
	 * 
	 * public LeaveItemDao getInstance() { return leaveItemDao; }
	 * 
	 * private LeaveItemDao() {
	 * 
	 * }
	 * 
	 * public LeaveItem selectUsableByCategory(Connection conn, String category)
	 * throws SQLException { PreparedStatement pstmt = null; ResultSet rs = null;
	 * String sql =
	 * "SELECT * FROM LEAVE_ITEM WHERE ITEM_NAME LIKE ? AND USE_YN='Y'"; try { pstmt
	 * = conn.prepareStatement(sql); pstmt.setString(1, "%" + category); rs =
	 * pstmt.executeQuery(); LeaveItem li = null; if (rs.next()) { li =
	 * convertLeaveItem(rs); } return li; } finally { JdbcUtil.close(rs);
	 ...
	 * li.setUseYn(rs.getString("USE_YN")); return li; }
	 */
