...
public class DailyWorkRecordDao {
	...
	public List<DailyWorkDetailDto> selectDetailByRequest(Connection conn, DailyWorkDetailRequest req)
		...
		String sql = "SELECT p.project_name AS PROJECT_NAME, dwr.work_date AS WORK_DATE, e.emp_no AS EMP_NO, e.emp_name_kr AS EMP_NAME_KR, "
				...
				+ "FROM daily_work_record dwr " + "LEFT JOIN employee e ON dwr.employee_id = e.employee_id "
				+ "LEFT JOIN department d ON e.department_id = d.department_id "
				+ "LEFT JOIN project p ON dwr.project_id = p.project_id "
				+ "WHERE (? IS NULL OR dwr.work_date BETWEEN TO_DATE(?, 'YYYYMMDD') AND TO_DATE(?, 'YYYYMMDD')) "
				+ "AND (? IS NULL OR e.emp_name_kr LIKE '%' || ? || '%') " + "AND (? IS NULL OR d.department_id = ?) "
				+ "AND (? IS NULL OR p.project_id = ?) " + "ORDER BY dwr.work_date DESC";
	 ...
	 * 계산 支給額・控除額・期間値を業務基準に従って計算し、合計と最終金額へ反映する。
	 */
	...
	private DailyWorkListDto convertDailyWorkListDto(Connection conn, ResultSet rs, int employeeId, String start,
		...
		String sql = "SELECT dwr.*, e.EMP_NAME_KR, p.PROJECT_NAME " + "FROM DAILY_WORK_RECORD dwr "
