...
import erp.hr.model.EmpLicense;
...
public class EmpLicenseDao {
    ...
    public void insert(Connection conn, EmpLicense license) throws SQLException {
    ...
    public EmpLicense selectById(Connection conn, int licId) throws SQLException {
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
    ...
    public List<EmpLicense> selectByEmpId(Connection conn, int empId) throws SQLException {
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
            String sql = "SELECT LIC_ID, EMP_ID, LIC_NAME, ACQ_DATE, ISSUER, LICENSE_NO, NOTE "
            ...
            List<EmpLicense> result = new ArrayList<>();
            while (rs.next()) {
    ...
    public int update(Connection conn, EmpLicense license) throws SQLException {
        PreparedStatement pstmt = null;
        try {
    ...
    // ResultSet 데이터를 EmpLicense 객체로 변환
    // 코드 중복 방지를 위한 공통 매핑 처리
    private EmpLicense makeEmpLicenseFromResultSet(ResultSet rs) throws SQLException {
        EmpLicense license = new EmpLicense();
