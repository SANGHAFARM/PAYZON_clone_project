...
import erp.hr.model.EmpAppointment;
...
public class EmpAppointmentDao {
    ...
    public void insert(Connection conn, EmpAppointment app) throws SQLException {
    ...
    public EmpAppointment selectById(Connection conn, int appId) throws SQLException {
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
    ...
    public List<EmpAppointment> selectByEmpId(Connection conn, int empId) throws SQLException {
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
            String sql = "SELECT APP_ID, EMP_ID, APP_TYPE, APP_DATE, DEPT_NAME, POS_NAME, JOB_TITLE_DUTY, NOTE "
            ...
            List<EmpAppointment> result = new ArrayList<>();
            while (rs.next()) {
    ...
    public int update(Connection conn, EmpAppointment app) throws SQLException {
        PreparedStatement pstmt = null;
        try {
    ...
    // ResultSet 데이터를 EmpAppointment 객체로 변환
    // 코드 중복 방지를 위한 공통 매핑 처리
    private EmpAppointment makeEmpAppointmentFromResultSet(ResultSet rs) throws SQLException {
        EmpAppointment app = new EmpAppointment();
